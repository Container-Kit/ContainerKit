<script lang="ts">
    ...
    import {
        type PullImageDetails,
        PullImageSchema,
        type PullImageValidatedDataType
    ...
    async function initiateImagePull() {
        ...
        if (!validatedImagePullData.success) {
            ...
            return toast.error(
                `Error while validating ${validatedImagePullData.error.issues.map((issue) => issue.path.join('.')).join(', ')}`
            );
        }
        ...
        if (imageDetails.platform) {
            ...
            toast.warning(
                'Platform and arch both specified. Platform will take precedence over arch.'
            );
        }
        ...
        const result = await pullImage(options);
        if (!result.ok) {
            imageDetails.pulling = 'none';
            return toast.error(`Failed to pull ${imageDetails.name}:${imageDetails.tag} image`, {
    ...
    <Dialog.Content>
        ...
        {#if isDisabled}
            <div class="flex flex-col" transition:slide>
                <Alert.Root
                    class={[
                        imageDetails.pulling === 'progress' && 'text-indigo-500',
                        imageDetails.pulling === 'done' && 'text-green-500'
                    ]}
                >
                    {#if imageDetails.pulling === 'done'}
                        <CheckCircle2Icon />
                        <Alert.Title
                            >Success! Your {imageDetails.name} image have been pulled</Alert.Title
                        >
                    {:else}
                        <Spinner />
                        <Alert.Title
                            >Please wait while we are pulling your {imageDetails.name} image.</Alert.Title
                        >
                    {/if}
                </Alert.Root>
            </div>
        {/if}
        <form
            onsubmit={initiateImagePull}
            onchange={() => (validatedImagePullData = PullImageSchema.safeParse(imageDetails))}
        >
            <Field.Set>
                <Field.Group>
                    ...
                    <Field.Set>
                        ...
                        <Field.Description>
                            Scheme to use when connecting to the container registry</Field.Description
                        >
                        {#if validatedImagePullData?.error?.issues}
                            {@const errors = validatedImagePullData.error.issues.filter(
                                (issue) => issue.path.join('.') === 'scheme'
                            )}
                            {#each errors as error}
                                <Field.Error>
                                    {error.message}
                        ...
                        <RadioGroup.Root
                            disabled={isDisabled}
                            id="scheme"
                            bind:value={imageDetails.scheme}
                        >
                    ...
                    <Field.Field orientation="horizontal">
                        ...
                        <Switch
                            disabled={isDisabled}
                            bind:checked={imageDetails.debug}
                            id="debug"
                        />
                    </Field.Field>
